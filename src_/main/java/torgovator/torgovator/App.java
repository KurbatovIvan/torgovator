	...
	private static void XmlDocumentParseAndInsert(String[] list) {
		...
		for (String dirItem : list) {
			if (!dirItem.contains("nsiOrganization")) {
				try {
					XmlDocumentZakupka Doc;
					try {
						Doc = new XmlDocumentZakupka(Params.getWorkDirTorgi().getWorkDirTmp() + "/" + dirItem);
						Doc.zakupka.setFileName(dirItem);
						try {
							//							data.InsertZakupki(Doc.zakupka);
							data.InsertData(Doc.getDocToInsert());
					...
					log.warning(ExceptionUtils.ExceptionStackToString(e1));
				...
				try {
					XMLnsiOrganizationList OrganizationList;
					try {
						OrganizationList = new XMLnsiOrganizationList(
								Params.getWorkDirTorgi().getWorkDirTmp() + "/" + dirItem);
						try {
							//							data.InsertOrg(OrganizationList.OrgList, dirItem);
							data.InsertData(OrganizationList.getDocToInsert());
						} catch (SQLException e) {
							log.warning(ExceptionUtils.ExceptionStackToString(e));
						}
					} catch (IOException e) {
